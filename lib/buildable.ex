defmodule Buildable.Behaviour do
  @moduledoc """
  A module that extends the protocol `Buildable` defining callbacks where the first argument is not a buildable.

  See the source code for `Buildable.Implementation.__using__/1` to see how the boiler-plate functions
  are implemented.
  """
  ...
  @callback extract(Buildable.t()) ::
              {:ok, Buildable.element(), updated_buildable :: Buildable.t()} | :error

  @callback insert(Buildable.t(), term) :: updated_buildable :: Buildable.t()

  @callback peek(Buildable.t()) :: {:ok, Buildable.element()} | :error

  @callback peek(Buildable.t(), Buildable.position()) :: {:ok, Buildable.element()} | :error
